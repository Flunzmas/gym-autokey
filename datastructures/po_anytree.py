...
def parse_obligation_ast(content):
    ...
    ast_anytree_root_node = Node(
        po_id, op_name="RootImp", op_class="RootImp", id=node_id, depth=0)
    node_id += 1
    parse_second_level_ast(
        content, ast_anytree_root_node, "antecedent", "and", 1)
    parse_second_level_ast(
        content, ast_anytree_root_node, "succedent", "or", 1)
    return ast_anytree_root_node


def parse_second_level_ast(content, root_node, key, junctor_name, depth):
    """
    This method is needed because the succedent/antecedent nodes of a goal AST
    ...
    if len(subtree) == 1:
        ...
        junctor_node = Node(
            junctor_name, op_name=junctor_name, op_class="Junctor",
            id=node_id, depth=depth, parent=root_node)
        node_id += 1
...
def _parse_obligation_ast_recursive(content, parent_node, depth):
    """
    Recursively adds children to given parent node until there is
    no more content.
    """
    global node_id
    op_n = content["op"]

    # underscore because given that way by the interface
    op_c = content["op_class"].split(".")[-1]
    cur_node = Node(
        op_n, op_name=op_n, op_class=op_c, id=node_id,
        depth=depth, parent=parent_node)
    node_id += 1
...
def print_anytree(root_node):
    ...
    for pre, _, node in RenderTree(root_node):
        print(
            '{pre} id = {id} | d = {d} | {n} ({c})'.format(
                pre=pre, id=node.id, d=node.depth,
                n=node.op_name, c=node.op_class))
    print("")
...
def ast_anytree_to_apted(cur_node):
    ...
    return Tree(
        cur_node.op_name + " " + cur_node.op_class, *children_apted_nodes)
